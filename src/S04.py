...
# Global Constants
from config import DEFECT_CATEGORY_MAP, S04_MESSAGE_CODE, SORT_CODE_MAP, WINDOW_TIME
...
temp_df["timeStamp"] = pd.to_datetime(
    temp_df["timeStamp"], format="%y%m%d%H%M%S%f", errors="coerce"
)

# Droping records that are not "54177" (S04) in messageCode column
original_records = len(temp_df)
temp_df = temp_df[temp_df["messageCode"] == S04_MESSAGE_CODE]
...
temp_df["rawMessage"] = (
    temp_df["rawMessage"].str.removeprefix("->{").str.removesuffix("}<")
)
...
for col in [
    ...
    "timeStamp",
    "barcodeAWCS",
]:  # don’t drop sortCode or indexNo
...
do_cleanup = (
    input("Do you want to clean up wrong sortCodes using the Excel file? (yes/no): ")
    .strip()
    .lower()
)
...
if do_cleanup != "yes":
    ...
    try:
        bad_ids_df = load_data()  # Always returns a DataFrame
    except ValueError:
        print("\nNo file selected. Skipping Cleanup\n")
        bad_ids_df = None

    if bad_ids_df is not None:
        ...
        modified_count = 0
        matched_ids = []
        used_rows = set()  # track rows already modified
        ...
        for bad_id, comment in zip(bad_ids_df[id_col], bad_ids_df[comment_col]):
            ...
            # Candidate rows not already used
            ...
            if len(candidates) > 0:
                row_idx = candidates[0]  # take first available row
                window_df.at[row_idx, "sortCode"] = 0
                window_df.at[row_idx, "No Scan Defect Explanation"] = (
        ...
        not_found = [
            ...
            if pd.notna(x) and str(int(x)).zfill(4) not in matched_ids
        ]
...
map_series = window_df["requestedDestMCID"].map(mapping_destination_names)
...
normalUniquePackages = window_df.loc[
    ~(mask_no_read | mask_multi_read), "barcodeAWCS"
].nunique()
...
defect_summary["percentage"] = (defect_summary["count"] / total_processed * 100).round(
    4
)
...
with pd.ExcelWriter(output_path, engine="xlsxwriter") as writer:
    ...
    reason_dest_pivot.to_excel(
        writer, sheet_name="Analysis_Results", startrow=31, startcol=0, index=False
    )
    ...
    ws.insert_chart(
        start_row_pivot + len(reason_dest_pivot) + 1,
        0,
        stack_chart,
        {"x_scale": 8, "y_scale": 3},
    )
